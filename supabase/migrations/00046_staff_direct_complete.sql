-- Ticket 0049 (ADR-0020): staff may complete assigned tasks from not_started when prerequisites pass.
...
BEGIN
  ...
  ELSIF p_new_status = 'completed'::public.task_status THEN
    IF v_task.status = 'not_started'::public.task_status
       OR v_task.status = 'in_progress'::public.task_status THEN
    ...
    ELSE
      RAISE EXCEPTION 'INVALID_STATE_TRANSITION: task must be not_started or in_progress before completing'
        USING ERRCODE = 'P0001';
    END IF;
  ELSE
